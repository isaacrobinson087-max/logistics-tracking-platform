...
const statusOptions = ['Shipment created', 'Received at facility', 'In transit', 'Customs processing', 'Out for delivery', 'Delivered', 'Delayed']
...
const demoShipment = {
  ...
  events: [
    { status: 'In transit', location: 'International Transit Hub', note: 'Shipment is moving to the destination country.', date: '21 Sep 2026', time: '10:30' },
    { status: 'Received at facility', location: 'Lagos Processing Centre', note: 'Shipment received and processed.', date: '20 Sep 2026', time: '16:45' },
    { status: 'Shipment created', location: 'Lagos, Nigeria', note: 'Shipment information received.', date: '20 Sep 2026', time: '09:15' },
  ],
}
  ...
  return <header className="site-header">
    <div className="nav-wrap">
      ...
      <nav className={open ? 'nav-open' : ''} aria-label="Main navigation">
        ...
        <button className="nav-track" onClick={() => { onTrack(); setOpen(false) }}>Track shipment <ArrowRight size={18} /></button>
      </nav>
...
function TrackingSearch({ initial = '', large = false, onResult }) {
  ...
  async function submit(e) {
    ...
    try {
      if (!isFirebaseConfigured && number === demoShipment.trackingNumber) return onResult(demoShipment)
      ...
      if (!snapshot.exists()) throw new Error('We could not find that tracking number. Please check it and try again.')
      onResult(snapshot.data())
    } catch (err) { setError(err.message) }
    finally { setLoading(false) }
  }

  return <form className={`tracking-form ${large ? 'tracking-form-large' : ''}`} onSubmit={submit}>
    <label htmlFor={large ? 'hero-tracking' : 'page-tracking'}>Tracking number</label>
    <div className="tracking-input-row">
      <span className="input-icon"><Search size={22} /></span>
      <input id={large ? 'hero-tracking' : 'page-tracking'} value={value} onChange={e => setValue(e.target.value)} placeholder="e.g. DEH-2026-000001" autoComplete="off" />
      <button type="submit" disabled={loading}>{loading ? 'Searching…' : 'Track package'} <ArrowRight size={20} /></button>
    </div>
    {error && <p className="form-error"><CircleAlert size={18} />{error}</p>}
    {!isFirebaseConfigured && <p className="demo-hint">Preview tracking number: <button type="button" onClick={() => setValue(demoShipment.trackingNumber)}>DEH-2026-000001</button></p>}
  </form>
}
  ...
  return <>
    ...
    <main>
      <section className="hero" id="home">
        <div className="hero-grid">
          <div className="hero-copy">
            ...
            <p className="hero-lead">Simple, dependable logistics for packages moving across countries. Know where your shipment is, every step of the way.</p>
          ...
          <div className="route-card" aria-label="International delivery illustration">
            ...
            <div className="route-places"><span><small>FROM</small>Lagos</span><strong>→</strong><span><small>TO</small>London</span></div>
      ...
      <section className="section services" id="services">
        <div className="section-heading"><p className="eyebrow">What we do</p><h2>Delivery solutions made simple.</h2><p>Whether it is one important package or regular business shipments, we help move it safely and keep you informed.</p></div>
      ...
      <section className="section about" id="about">
        <div className="about-panel"><p className="eyebrow">Why Dehub</p><h2>Logistics without the confusion.</h2><p>We believe shipment information should be easy for everyone to understand. Dehub combines dependable coordination with straightforward tracking, so customers can check progress without unnecessary steps.</p><button onClick={() => navigate('track')}>Track a shipment <ChevronRight /></button></div>
        <div className="steps">
...
function Footer({ navigate }) {
  return <footer><div className="footer-grid"><Brand light /><p>Dependable logistics and clear shipment tracking across borders.</p><button onClick={() => navigate('admin')} className="admin-link"><UserRound size={17} /> Staff login</button></div><div className="footer-bottom"><span>© 2026 Dehub Logistics Services</span><span>Built for clear, dependable delivery.</span></div></footer>
}
...
function TrackingPage({ navigate, initialShipment }) {
  ...
  return <div className="light-page">
    ...
    <main className="tracking-page">
      <button className="back-link" onClick={() => navigate('home')}><ArrowLeft size={18} /> Back to home</button>
      <div className="track-page-heading"><p className="eyebrow">Shipment tracking</p><h1>Where is your package?</h1><p>Enter the tracking number supplied with your shipment.</p></div>
...
function ShipmentResult({ shipment }) {
  ...
  return <section className="shipment-result">
    <div className="status-banner"><span className="status-icon"><Truck /></span><div><small>CURRENT STATUS</small><h2>{shipment.status}</h2><p>Last updated {shipment.updatedAt || 'recently'}</p></div><span className="status-pill">On schedule</span></div>
    ...
    <div className="journey"><div className="journey-title"><div><h2>Shipment journey</h2><p>{shipment.origin} to {shipment.destination}</p></div><Box /></div>
      <div className="timeline">{events.map((event, i) => <div className={`event ${i === 0 ? 'active' : ''}`} key={`${event.date}-${i}`}><span className="event-dot">{i === 0 ? <Truck size={17} /> : <Check size={16} />}</span><div className="event-body"><div><h3>{event.status}</h3><p><MapPin size={16} />{event.location}</p>{event.note && <p className="event-note">{event.note}</p>}</div><time>{event.date}<small>{event.time}</small></time></div></div>)}</div>
    </div>
  </section>
}
...
function Login({ navigate, onDemoLogin }) {
  ...
  async function submit(e) {
    ...
    try {
      if (!isFirebaseConfigured) { onDemoLogin(); return }
      await signInWithEmailAndPassword(auth, email, password)
    } catch { setError('The email or password is incorrect. Please try again.') }
    finally { setLoading(false) }
  }
  return <div className="login-page"><div className="login-brand"><Brand light /><button onClick={() => navigate('home')}><ArrowLeft /> Return to website</button></div><div className="login-panel"><div className="login-card"><span className="login-icon"><ShieldCheck /></span><p className="eyebrow">Authorised staff only</p><h1>Admin sign in</h1><p>Manage shipments and customer tracking updates.</p><form onSubmit={submit}><label>Email address<input type="email" required value={email} onChange={e => setEmail(e.target.value)} placeholder="admin@company.com" /></label><label>Password<input type="password" required value={password} onChange={e => setPassword(e.target.value)} placeholder="Enter your password" /></label>{error && <p className="form-error"><CircleAlert size={18} />{error}</p>}<button type="submit">{loading ? 'Signing in…' : 'Sign in securely'} <ArrowRight /></button></form>{!isFirebaseConfigured && <p className="demo-login">Preview mode: enter any email and password.</p>}</div></div></div>
}
...
function Dashboard({ navigate, demo, user }) {
  ...
  useEffect(() => {
    ...
    getDocs(query(collection(db, 'shipments'), orderBy('createdAt', 'desc'), limit(50)))
      ...
      .catch(err => setError(`Could not load shipments: ${err.message}`))
  }, [demo])
  ...
  async function save(e) {
    ...
    const record = { ...form, trackingNumber, updatedAt: new Date().toLocaleString('en-GB'), events: [{ status: form.status, location: form.origin, note: form.publicNote || 'Shipment information received.', date: new Date().toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' }), time: new Date().toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit' }) }] }
    try {
      if (demo) setShipments(s => [{ id: trackingNumber, ...record }, ...s])
      else {
        if (!db || !user) throw new Error('Your administrator session is not available. Please sign in again.')
        const batch = writeBatch(db)
        const privateRef = doc(db, 'shipments', trackingNumber)
        const publicRef = doc(db, 'public_tracking', trackingNumber)
        batch.set(privateRef, { ...record, createdAt: serverTimestamp(), updatedAtServer: serverTimestamp(), createdBy: user.uid })
        batch.set(publicRef, { ...record, updatedAtServer: serverTimestamp() })
        await batch.commit()
        setShipments(s => [{ id: trackingNumber, ...record }, ...s.filter(item => item.id !== trackingNumber)])
      }
      setForm(emptyForm); setFormOpen(false); setNotice('Shipment created successfully and saved permanently.'); setTimeout(() => setNotice(''), 3500)
    } catch (err) {
      setError(`Shipment was not saved: ${err.message}`)
    } finally {
      setSaving(false)
    }
  }
  ...
  return <div className="dashboard">
    ...
    <main className="dash-main"><header><div><p>ADMIN DASHBOARD</p><h1>Shipment overview</h1></div><button className="primary" onClick={() => { generateNumber(); setFormOpen(true) }}><Plus /> Create shipment</button></header>
      ...
      {error && !formOpen && <p className="form-error"><CircleAlert size={18} />{error}</p>}
...
export default function App() {
  ...
  function navigate(next, data) { setPage(next); if (data) setShipment(data); window.scrollTo({ top: 0, behavior: 'smooth' }) }
  if (page === 'home') return <Home navigate={navigate} />
  if (page === 'track') return <TrackingPage navigate={navigate} initialShipment={shipment} />
  if (page === 'admin' && !user && !demoAdmin) return <Login navigate={navigate} onDemoLogin={() => setDemoAdmin(true)} />
  if (page === 'admin' || user || demoAdmin) return <Dashboard navigate={navigate} user={user} demo={demoAdmin || !isFirebaseConfigured} />
  return null
}
